...
In this section we overview the field of modelling and simulation of physical systems by using a simple but illustrative example. We model and simulate the physical system and through this process introduce basic concepts of modelling and simulation. When necessary, we abstract from the concrete example and define the concepts generally.
...
The first four equations describe the behaviour of the components. The last three equations describe the topology of the circuit. The system of equations consists of implicitly defined algebraic and differential equations. This mathematical representation is called system of implicit \emph{differential algebraic equations (DAEs)} \cite{Cellier2006}. More generally, system of implicit DAEs can be written in the following way:

\begin{equation}
f(\frac{d\vec{x}}{dt},\vec{x},\vec{y},t) = 0
\end{equation}

where $\vec{x}$ is a vector of \emph{differential variables}, also known as \emph{state variables}, $\vec{y}$ is a vector of algebraic variables and $t$ is an independent scalar variable, in physical modelling it represents the \emph{time}.

In general, it is not possible to find an exact solution of a DAE using analytical methods. Approximate solutions are derived by \emph{numerical integration}. Their are a number of methods for numerical integration of an implicit DAE. They can be divided into two categories. Solvers in the first category operate directly on the implicit DAE. Solvers in the second category transform the implicit representation into the explicit one and operate on the explicit representation. In this section we illustrate the later.


\subsection{Symbolic Manipulation}

In order to transform this implicit DAE into an explicit one, we perform the following steps. Firstly, we identify \emph{known} and \emph{unknown} variables. Secondly, we decide which unknown variable should be solved in which equation. Thirdly, we sort equations in a way that no unknown variable is used before it is solved.

Time $t$ and state variables ($u_c$ and $i_2$), are assumed to be known, the rest of the variables are unknowns including the state derivatives ($\frac{du_c}{dt}$ and $\frac{di_2}{dt}$). Equations that contain only one unknown are solved for it. After that, solved variables are assumed to be known and rest of the variables are solved. In this case this technique suffices and we get the following explicit DAE:
...
This symbolic manipulation process is called \emph{causalisation}. Note that now the cause-effect relationship is explicitly specified which was not the case for the implicit DAE.

Let us substitute the variables defined in the first five equations in the last two equations. This effectively eliminates the algebraic equations from the system.
...
This representation is called system of explicit \emph{ordinary differential equations (ODEs)} and can be passed to a numerical ODE solver. This representation is also called \emph{state-space model}. More generally, a system of explicit ODEs can be written in the following way:
...
In the following we explain the simplest numerical integration method for ODEs, i.e. the \emph{forward Euler} method. The key idea is to replace the derivatives with the following approximation:
...
More accurate and efficient numerical integration methods are available based on different approximations and integration algorithms. More comprehensive presentation of this and other more sophisticated methods can be found in \cite{Cellier2006}.
...
Block-diagrams in causal languages correspond to systems of ordinary differential equations (ODEs) in explicit form; i.e., the causality is explicit, the input and state variables are used to define the state derivatives and the output variables. The construction of a block-diagram is closely related to the process of causalisation (see Section \ref{secModelling}).
...
The |TwoPin| model not only defines the variables, but also the non-causal equations they satisfy. The variables |p| and |n| represent the positive and negative pins of an electrical component. The variable |u| represents the voltage drop across the component. The variable |i| represents the current flowing into the positive pin.

By extending the |TwoPin| model and adding component-specific equations we define the model representing resistor, capacitor, inductor and voltage source models. Note the use of \emph{inheritance} to reuse the equations from the |TwoPin| model.
...
We can now use models of the circuit components to define a model representing the circuit in Figure \ref{figSimpleCircuit} by ``connecting'' appropriate pins according to the figure. Note that the parameters of resistor, capacitor, inductor and voltage source are not specified. This means that the default values are used.
...
Connected potential variables generate equality constraints stating that all connected potential variables are equal at any point in time. For |SimpleCircuit| model a Modelica compiler generates the following equations:
