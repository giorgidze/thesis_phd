...
signal, signal function and signal relation. The conceptual definitions are
provided to facilitate development of and reasoning about Hydra models. The
actual implementation of theses concepts is presented in Chapter
\ref{chapImplementation}. This chapter also informally introduces Hydra
language by using illustrative examples. The formal definition of the language
...
Conceptually, a \emph{signal} is a time-varying value; i.e., a function from
time to value:
...
type parameter |alpha| specifies the type of values carried by the signal. For
example, a signal that corresponds to the amount of current flowing in an
electrical circuit has type |Signal Double|, while a signal that corresponds
to the position of a certain object in a two dimensional space has type
|Signal (Double,Double)|.

The basic signal types that are supported in Hydra are: |Signal Double| and
|Signal Bool|. Signals of arbitrarily nested tuples of basic types are also
supported. As an example of a signal that carries nested tuples we consider a
signal of type |Signal ((Double,Double),(Double,Double))| that can be used to
represent current and voltage pairs at the positive and negative pins of a
two-pin electrical component.
...
Because a product of signals, say |Signal alpha| and |Signal beta|, is
isomorphic to a signal of the product of the carried types, in this case
|Signal (alpha, beta)|, unary signal functions actually suffice for handling
signal functions of any arity. For example, the binary signal function
|plus_sf| that takes two signals and computes sum of their values at each
point in time can be given the following type and definition:
...
mathematical operations (e.g., |+|, |*|, |sin| and |cos|) to signal level.
Hydra also provides |der :: SF Double Double| signal function that
differentiates the given signal. This signal function enables definition of
...
given values are related or not. The same is of course true for signal
relations:
...
type of propositions defined in the first-order logic. \emph{Solving} a
...
relation that can be given the following type and definition:

\begin{code}
(=) ::  SR (alpha,alpha)
(=) t0 s ~= forall t, t >= t0 => fst (s t) == snd (s t)
\end{code}
