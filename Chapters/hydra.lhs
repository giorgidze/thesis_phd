...
In this section we describe the three central concepts Hydra is based on: signal, signal function and signal relation. The conceptual definitions are only provided to facilitate reasoning about Hydra models. The actual implementation of theses concepts is presented in Chapter \ref{chapImplementation}.
...
Conceptually, a \emph{signal relation} is simply a relation on signals. Stating that some signals are in a particular relation to each other imposes \emph{constraints} on those signals. Assuming these constraints can be satisfied, this allows some of the signals to be determined in terms of the others depending on which signals are known and unknown in a given context. That is, signal relations are non-causal, unlike signal functions where the knowns and unknowns (inputs and outputs) are given a priori.
...
There are two levels to Hydra: the \emph{functional level} and the \emph{signal level}. The functional level is concerned with the definition of ordinary functions operating on time-invariant values. The signal level is concerned with the definition of signal relations and signal functions.

Signal relations and signal functions are \emph{first-class} entities at the functional level. Signals, in contrast, are \emph{not} first-class entities at the functional level. However, crucially, \emph{instantaneous values} of signals can be propagated back to the functional level, allowing the future system structure to depend on signal values at discrete points in time, as we explain in later in this section in more detail.
...
abstract syntax tree (AST), that then is compiled into simulation code.
...
The symbols | [rel|| | and | ||] | are the quasiquotes. At compile time, GHC applies the user-defined (in this case defined by the embedded language developer) parsing function named in the opening quote to the text between the quotes. Here, the function is called |rel|. It has type |String -> SR a| and parses the concrete version of the Hydra syntax defined in Section~\ref{secSyntax}. Values of type |SR a| are ASTs representing Hydra signal relations. This enables the embedded Hydra compiler to process them symbolically and ultimately compile them into simulation code.
...
Just like for signal relations we use quasi-quoting for defining signal relations. The pattern binds \emph{signal variables} that scope over the expression that follows. Signal functions can be applied to signals:

\begin{code}
sf s
\end{code}

Signal function applications must be well typed. In this example, if |sf| has type |SF alpha beta|, |s| must have type |Signal alpha|. The type of resulting signal is |Signal beta|.

\section{Structurally Dynamic Signal Relations}

Primitive, built-in equality signal relation (i.e., |=|) is capable of describing flat systems of equations. On the other hand, signal relation application operator (i.e., |<>|) provides for hierarchically structured systems of equations. In this section we introduce one more built-in (higher-order) signal relation that allows description of structurally dynamic signal relations:

\begin{code}
switch :: SR a -> SF a Bool -> (a -> SR a) -> SR a
\end{code}

The |switch| combinator, which forms signal relations by temporal composition, takes three arguments that define the composite signal relation (of type |SR a|) returned by the combinator. The first argument (of type |SR a|) is a signal relation that is initially active. The second argument is a signal function (of type |SF a Bool|) that computes a signal of type |Signal Bool| given the signal (of type |Signal a|) constrained by the composite signal relation. Starting from the point in time when the boolean signal equals |True| for the first time the composite behaviour is defined by the signal relation that is computed by applying the third argument (a function of type |a -> SR a|) to the instantaneous value of the constrained signal.
...
Let us introduce the Haskell embedding of Hydra by modelling the circuit in Figure \ref{figSimpleCircuit}. We first define a |twoPin| model: a signal relation that captures the common behaviour of electrical components with two connectors (see Figure \ref{figTwoPin}):
...
The signal variables |p_i| and |p_v| represent the current into the positive pin and the voltage at the positive pin. The signal variables |n_i| and |n_v| represent the current into the negative and the voltage at the negative pin. The signal variable |u| represents the voltage drop across the electrical component. Note that |u| is declared as local signal variables, that is, it is not exposed in the interface of the signal relation. As a consequence, |u| can not be constrained further unlike the rest of the variables in the interface.

Signal variables in the |rel| pattern qualified as |flow| are called \emph{flow} signal variables. Signal variables without any qualifier are called \emph{potential} signal variables. |flow| is just a syntactic sugar that facilitates easy and straight forward definition of connection equations as discussed in Section~\ref{subSecConnections}.
...
Note the two kinds of variables: the functional level ones representing \emph{time-invariant} parameters, and the signal-level ones, representing \emph{time-varying} entities, the signals. Functional-level fragments, such as variable references, are spliced into the signal level by enclosing them between antiquotes, \$. On the other hand time-varying entities are not allowed to escape to the functional level (meaning signal-variables are not in scope between antiquotes).
...
In the setting of Hydra, the |connect| construct is just syntactic sugar with the following rules:
...
\item Connection of flow signal variables with potential signal variables is not allowed.

\end{itemize}

For connected flow variables, sum-to-zero equations are generated. For potential variables, equality constraints are generated. The connect constructs of |simpleCircuit| are thus expanded to the following equations:
...
To introduce structurally dynamic modelling in Hydra, let us model a physical system whose structural configuration changes abruptly during simulation: a simple pendulum that can break at a specified point in time; see Fig. \ref{fig:pendulum}. The pendulum is modelled as a body represented by a point mass $m$ at the end of a rigid, mass-less rod, subject to gravity $m \vec{g}$. If the rod breaks, the body will fall freely. This makes the differences between the two configurations sufficiently large that e.g. Modelica does not support non-causal modelling of this system. Instead, if simulation across the breaking point is desired, the modeller is forced to model the system in a causal, less declarative way \cite[pp. 31--33]{ModelicaTutorial2000}.
